...
/**
 * What the passport does NOT specify yet — shown honestly so the shopper
 * knows which dimensions the matching engine can't use for them.
 */
export function passportMissingInfo(passport: AdaptiveFitPassport): string[] {
  const a = passport.answers;
  const missing: string[] = [];
  const country = a.country?.[0];
  if (!country || country === "Other country") missing.push("Shopping region");
  if ((a.clothing ?? []).length === 0 || (a.clothing ?? []).some((c) => c.startsWith("Not sure")))
    missing.push("Clothing categories");
  if ((a.help ?? []).filter((h) => !/not listed/i.test(h)).length === 0)
    missing.push("Functional needs");
  if (!a.range?.[0] || a.range[0].startsWith("Prefer")) missing.push("Clothing range");
  if ((a.style ?? []).length === 0) missing.push("Style preference");
  if (!a.budget?.[0] || a.budget[0] === "No preference") missing.push("Budget");
  return missing;
}
...
/**
 * A plain-markdown export of the passport, for copying or downloading —
 * useful when talking to a brand, carer or occupational therapist. Framed
 * as shopping preferences, never as a medical record.
 */
export function passportToMarkdown(passport: AdaptiveFitPassport): string {
  const lines: string[] = [];
  const wearer = passportWearer(passport);
  const updated = new Date(passport.updatedAt).toLocaleDateString(undefined, {
    day: "numeric",
    month: "long",
    year: "numeric",
  });

  lines.push("# Adaptive Fit Passport");
  lines.push("");
  lines.push(`A personal adaptive clothing profile${wearer ? ` for: ${wearer}` : ""}.`);
  ...
  if (mustHaves.length > 0) {
    ...
    lines.push("");
    lines.push("These are hard requirements — never traded away for style or budget.");
    lines.push("");
    mustHaves.forEach((item) => lines.push(`- ${item}`));
    lines.push("");
  }

  passportSections(passport).forEach((section) => {
  ...
  lines.push("");
  lines.push(
    "This passport describes clothing and shopping preferences the wearer " +
      "selected themselves. It is not a medical record, assessment or " +
      "diagnosis. Generated by Xi's Adaptive Clothing; stored only on the " +
      "wearer's own device."
  );
  return lines.join("\n");
}
